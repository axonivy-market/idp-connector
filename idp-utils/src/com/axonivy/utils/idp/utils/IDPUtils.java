...
import static com.axonivy.utils.idp.utils.Constants.STAND_ALONE_URL;
...
import com.axonivy.utils.idp.WorkflowType;
...
public class IDPUtils {
	...
	public static String getSharingUrl(UUID documentId, WorkflowType workflowType, String token) {
		String url = STAND_ALONE_URL.replace("{baseUrl}", Ivy.var().get("idpUtils.standAloneUrl"))
				.replace("{workflowType}", workflowType.getValue())
				.replace("{documentId}", documentId.toString())
				.replace("{token}", token);
		
		String confidenceMinValueStr = Ivy.var().get("idpUtils.confidenceMinValue");
